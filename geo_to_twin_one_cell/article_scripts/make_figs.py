...
fg, bg = find_fg_bg(image, verbose=False)

vol = pv.ImageData(dimensions=image.shape)
vol.point_data["values"] = image.T.flatten()
slices = pv.merge(vol.slice_along_axis(n=7, axis="z"))
...
pv_plotter.add_scalar_bar(
    title="CT scan graylevels\n",
    vertical=False,
    position_x=0.1,
    position_y=0.85,
    height=0.1,
...
setup_camera(pv_plotter)
pv_plotter.save_graphic("../figs/graylevel_image.pdf")
pv_plotter.show()
...
vmin, vmax = actor.mapper.scalar_range
pv_plotter.add_mesh(
    elem_mesh,
    color="#e6ab02",
    style="wireframe",
...
# %% simu: plot von mises on FEM mesh
...
cmap = plt.colormaps["jet"].resampled(n_colors)
warped_mesh = fe_mesh.warp_by_vector(vectors="U")
pv_plotter.add_mesh(
    warped_mesh,
    scalars="von_mises",
    cmap=cmap,
...
setup_camera(pv_plotter)
pv_plotter.save_graphic("../figs/FE_von_mises.pdf")
pv_plotter.show()
